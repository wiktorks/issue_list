...
class RegisterForm(FlaskForm):
    username = StringField('Username', validators=[DataRequired(), Length(min=5, max=30)])
    user_email = EmailField('Email', validators=[DataRequired(), Length(min=5, max=30), Email()])
    ...
    def validate_username(self, username):
        ...
        if user:
            raise ValidationError(message='Account with given username already exists. Try a different one.')
